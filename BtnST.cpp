...
// DrawTransparent should be called 
// just after the button is created.
// Do not use trasparent buttons until you really need it 
// (you have a bitmapped
// background) since each transparent button 
// makes a copy in memory of its background.
// This may bring unnecessary memory use
// and execution overload.
//
// Parameters:
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
...
void CButtonST::OnPaint() 
{
	...
	CRect rect, rt;
	GetClientRect(&rect);
	if(m_bIsCheckBox && GetCheck() == 1)
	{
		rt = rect; rt.bottom = rt.top + 3;
		dc.FillRect(&rt, &m_brBack);
		rt = rect; rt.right = rt.left + 3;
		dc.FillRect(&rt, &m_brBack);
		rt = rect; rt.top = rt.bottom - 3;
		dc.FillRect(&rt, &m_brBack);
		rt = rect; rt.left = rt.right - 3;
		dc.FillRect(&rt, &m_brBack);
	}
}

void CButtonST::SetCheckBtn(BOOL bCheckBTN)
{
