...
CComboListCtrl::~CComboListCtrl()
{
	CInPlaceComboTP::DeleteInstance();
	CInPlaceEditTP::DeleteInstance();  
}
...
CInPlaceComboTP* CComboListCtrl::ShowInPlaceList(int iRowIndex, int iColumnIndex, CStringList& rComboItemsList, 
											   CString strCurSelecetion /*= ""*/, int iSel /*= -1*/)
{
	...
	CInPlaceComboTP* pInPlaceCombo = CInPlaceComboTP::GetInstance();
...
CInPlaceEditTP* CComboListCtrl::ShowInPlaceEdit(int iRowIndex, int iColumnIndex, CString& rstrCurSelection)
{
	// Create an in-place edit control
	CInPlaceEditTP* pInPlaceEdit = CInPlaceEditTP::GetInstance();
		...
		if ((GetItemState(iRowIndex, flag ) & flag) == flag)
		{
			...
			if (GetWindowLong(m_hWnd, GWL_STYLE) & LVS_EDITLABELS)
			{
				...
				if (IsCombo(iColumnIndex))
				{
					...
					CInPlaceComboTP* pInPlaceComboBox = ShowInPlaceList(iRowIndex, iColumnIndex, obComboItemsList, strCurSelection);
				...
				{
					CInPlaceEditTP* pInPlaceEdit = ShowInPlaceEdit(iRowIndex, iColumnIndex, strCurSelection);
				}
			}
		}
	}  
